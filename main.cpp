...
{
	enum { NO_COLLISION = 0, TOP_OF_B = 1, RIGHT_OF_B = 2, BOTTOM_OF_B = 3, LEFT_OF_B = 4 };
				 ...
				 float rect_B_x, float rect_B_y, float rect_B_w, float rect_B_h, float rect_B_speed_x, float rect_B_speed_y, float b_inv_mass)
	{
		...
		float e = 10.0; // 1.0 is for elastic bounce, 0.0 for mud
	...
	*/
		...
		for (int i = 0; i <= n_boxes; i++)
			...
			// COLLISION NOT WORKING
			 impulse(box[i].x, box[i].y, box[i].w, box[i].h, box[i].speed_x, box[i].speed_y, 1,
				box[i+1].x, box[i+1].y, box[i+1].w, box[i+1].h, box[i+1].speed_x, box[i+1].speed_y, 1);
				...
				for (int k = 1; k <=4; k++)
				{
					impulse(box[i].x, box[i].y, box[i].w, box[i].h, box[i].speed_x, box[i].speed_y, 1, border[k].x, border[k].y, border[k].w, border[k].h);
